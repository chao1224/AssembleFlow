...
class PaiNN(nn.Module):
    ...
    def __init__(
        ...
        epsilon: float = 1e-8,
        extra_atom_features: bool = False,
    ):
        """
        Args:
            n_atom_basis: number of features to describe atomic environments.
        ...
        if extra_atom_features:
            ...
            self.embedding = ExtraAtomEmbedding(max_z, n_atom_basis)
        else:
    ...
    def forward_with_expanded_index(self, x, positions, radius_edge_index, batch, return_node_repr=False, return_vector=False):
        if radius_edge_index is None:
            from torch_geometric.nn import radius_graph
            radius_edge_index = radius_graph(positions, r=self.cutoff)

        if x.dim() == 1:
            q = self.embedding(x)[:, None]
        elif self.extra_atom_features: # When for the extra atom features
            q = self.embedding(x)[:, None]
        else:  # When the input z is one-hot, now only for JAE
            assert x.dim() == 2
            q = torch.matmul(x, self.embedding.weight)[:, None]
        qs = q.shape
        ...
        if return_vector and return_node_repr:
            return h, q, mu
        if return_vector:
            return h, mu
        if return_node_repr:
    ...
    def forward(self, gathered_x, positions, radius_edge_index, gathered_batch, periodic_index_mapping, return_node_repr=False, return_vector=False):
        idx_i, idx_j = radius_edge_index[0], radius_edge_index[1]
        r_ij = positions[idx_i] - positions[idx_j]
